...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    item = sandwiches.read_one(db, sandwich_id=sandwich_id)
    if item is None:
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    item = recipes.read_one(db, recipe_id=recipe_id)
    if item is None:
...
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    item = order_details.read_one(db, order_detail_id=order_detail_id)
    if item is None:
